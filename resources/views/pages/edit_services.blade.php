			...
			<div class="form-group">
				...
				<select id="multiselect" multiple="multiple" class="form-control" >
					@foreach ($types as $type)
					    <option value="{{$type->id}}" {{in_array($type->id, $typeIds) ? 'SELECTED' : '' }}>{{$type->name}}</option>
					@endforeach
				</select>
				</div>
			</div>
			<input type="hidden" name="selected_pet_types" id="multiselectOptions" value="" readonly="readonly"/>
			<input type="hidden" name="service_id" id="multiselectOptions" value="{{$service->id}}" readonly="readonly"/>
	</div> 
	<div class="panel-footer">
		<div class="row">
			<div class="col-xs-offset-1 col-md-offset-8 col-lg-offset-9 col-xs-11 col-md-4 col-lg-3">
				<button  type="button" class="btn btn-primary" id="service_update"
